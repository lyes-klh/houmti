...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  newUser = await User.findOne({ email: newUser.email }).select("+password");
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!passwordChecked) return next(new AppError("Wrong email or password", 400));
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!decoded) return next(new AppError("You are not logged in, please login to continue", 401));

  // check if user still exist
  const user = await User.findById(decoded.userId).select("+isAdmin");
  if (!user) return next(new AppError("This user does not exist, please login to continue", 401));

  // check if user changed password after jwt was issued
  const changedPassword = user.passwordChangedAt > decoded.iat * 1000;

  if (changedPassword) return next(new AppError("This token is not valid anymore", 401));
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) return next(new AppError("Email provided does not match with any account", 400));
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!email || !newPassword)
    return next(new AppError("Please provide your email and a new password", 400));

  const { token } = req.params;
  if (!token)
    return next(new AppError("Please provide a password reset token in the url parameters", 400));
  ...
  if (!correctToken || Date.now() > user.resetTokenExpires)
    return next(new AppError(`This reset token is not valid ${hashedToken}`, 400));
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!passwordChecked) return next(new AppError("Your password is wrong, please try again", 401));
...
exports.restrictToAdmin = catchAsync(async (req, res, next) => {
  if (!req.user.isAdmin) return next(new AppError("You can't access this route", 403));
  next();
});
